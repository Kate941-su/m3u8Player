...
class AVNodeMusicPlayer {
    
    // This values are inspired by Winamp
    static let cutoffFrequencies = [60, 170, 310, 600, 1000, 3000, 6000, 12000, 14000, 16000]
    static let shared = AVNodeMusicPlayer()

    private let audioEngine = AVAudioEngine()
    private let equalizer = AVAudioUnitEQ(numberOfBands: AVNodeMusicPlayer.cutoffFrequencies.count)
    ...
    func initializeAudioEngine() {
        do {
            ...
            for (index, frequency) in AVNodeMusicPlayer.cutoffFrequencies.enumerated() {
